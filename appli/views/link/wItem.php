...
<?php if(!empty($contextUserId) && $linkStatus != LINK_STATUS_BLACKLISTED) : ?>
    <?php if($contextUserId != $this->user['user_id']) : ?>
        <div class="linkDestinataire" data-destinataire-id="<?php echo $this->user['user_id']; ?>"  data-destinataire-login="<?php echo $this->user['user_login']; ?>" data-destinataire-message="<?php if(!empty($this->user['user_mail'])) echo $this->user['user_mail']; ?>" data-destinataire-photo="<?php echo $this->user['user_photo_url']; ?>">
                ...
                <?php elseif($linkStatus == LINK_STATUS_ACCEPTED) : ?>
                    <?php  //$destinataireId = ($this->link['expediteur_id'] == User::getContextUser('id')) $this->link['expediteur_id'] : $this->link['destinataire_id']; ?>

...
class ArticleController extends AppController
{
    ...
    public function render()
    {
        ...
        $article = $this->model->Article->getById($this->context->params['value']);
        ...
        $this->view->photos = $this->model->Photo->getPhotosByKey($this->context->params['value'], PHOTO_TYPE_ARTICLE);
    ...
    public function renderEdit($isValid = true)
    {
        ...
        if (!$isValid) {
            $this->view->article = $this->context->params;
        }
    ...
    public function renderSave()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->_isValid($this->context->params)) {
                $this->context->params['art_libel']       = htmlspecialchars($this->context->params['art_libel'], ENT_QUOTES, 'utf-8');
                $this->context->params['art_description'] = htmlspecialchars($this->context->params['art_description'], ENT_QUOTES, 'utf-8');
                if (!empty($this->context->params['art_id'])) {
                    if ($this->model->Article->updateArticle($this->context->params)) {
                    ...
                    $articleId = $this->model->Article->createArticle($this->context->params);
                    if ($articleId > 0) {
                        $this->view->growler('Article enregistré.', GROWLER_OK);
                        $this->context->params['value'] = $articleId;
                        $this->render();
                    } else {
    ...
    public function renderDelete()
    {
        ...
        $this->model->Article->deleteArticleById($this->context->params['value']);
